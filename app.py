...
def on_analyze_video(video_path, progress=gr.Progress()):
    if not video_path:
    ...
    final_path, uid_info = video_inference.process_video(
        video_path,
        detector,
        embedder_processor,
        embedder_session,
    ...
    status = (
        ...
        f"Detected and managed **{len(uid_info)}** unique identities across the video."
    )
...
with gr.Blocks(title="Identity Management Dashboard") as demo:
            ...
            with gr.Row():
                with gr.Column(scale=2, min_width=0):
                    with gr.Group():
                        ...
                        btn_process_vid = gr.Button("Run Video Detection", variant="primary")

                    with gr.Group():
                        gr.Markdown("### Video Output")
                        vid_out = gr.Video(label="Processed Output", interactive=False, show_label=False)
                
                with gr.Column(scale=1, min_width=360):
                    with gr.Group():
                        gr.Markdown("### Status & Summary")
                        vid_status_out = gr.Markdown("Upload a video and click 'Run Video Detection' to process it frame-by-frame.")
    ...
    btn_process_vid.click(
        fn=on_analyze_video,
        inputs=[vid_in],
        outputs=[vid_out, vid_status_out]
    )
